...
const TASK_META: Record<TaskType, { label: string; description: string }> = {
    reminders: {
        ...
        description: 'Notify students who have not submitted for a topic yet.',
    },
    'bulk-approve': {
...
function summarizeResult(task: ScheduledTask) {
    ...
    switch (task.taskType) {
        case 'reminders':
            return `${result.studentsNotified ?? 0} students notified`
        case 'bulk-approve':
...
function taskSummary(task: ScheduledTask) {
    switch (task.taskType) {
        case 'reminders':
            return task.config.topicName || 'Selected topic'
        case 'bulk-approve':
...
export default function TeacherSchedulerPage() {
    ...
    useEffect(() => {
        ...
        ;(async () => {
            try {
                await Promise.all([loadSchedulerData(), loadTopics()])
            } catch (error: any) {
    ...
    }, [ready, loadSchedulerData, loadTopics, showToast])
        ...
        <TeacherLayout title="Scheduler">
            ...
            <div className="max-w-6xl mx-auto p-6 space-y-8">
                <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">Create repeatable teacher workflows and run them locally in this app.</p>
                </div>
